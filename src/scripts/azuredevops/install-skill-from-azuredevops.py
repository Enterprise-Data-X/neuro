#!/usr/bin/env python3
"""Install a skill from a GitHub repo path into $NEURO_HOME/skills."""
...
def _neuro_home() -> str:
    return os.environ.get("NEURO_HOME", os.path.expanduser("~/.neuro"))


def _tmp_root() -> str:
    base = os.path.join(tempfile.gettempdir(), "neuro")
    os.makedirs(base, exist_ok=True)
    return base


def _request(url: str) -> bytes:
    return github_request(url, "neuro-skill-install")
...
def _default_dest() -> str:
    return os.path.join(_neuro_home(), "skills")
